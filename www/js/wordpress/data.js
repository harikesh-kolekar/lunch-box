...
app.factory('WordPress',['$http', 'Config', function($http, Config) {
	...
	data.getPosts = function (page) {
		return $http(
			{
				method: 'GET', url:'data/get_recent_posts.json?page='+page
			}
		);
	}
	data.getPostsTaxonamy = function (type, slug, page) {
		if(type == 'tag') {
			return $http(
				{
					method: 'GET', url:'data/get_tag_posts.json?page='+page+'&slug='+slug
				}
			);
		} else if(type == 'category') {
			return $http(
				{
					method: 'GET', url:+'data/get_category_posts.json?page='+page+'&slug='+slug
				}
			);
		}
	}
	data.getCategories = function () {
		return $http(
			{
				method: 'GET', url:'data/get_category_index.json'
			}
		);
	}
	data.getTags = function () {
		return $http(
			{
				method: 'GET', url:'data/get_tag_index.json'
			}
		);
	}
  	return data;
